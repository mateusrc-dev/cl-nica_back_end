...
class HoráriosSegundaController {
  ...
  async create(request, response) {
    ...
    await database.run("INSERT INTO horáriosSegunda (horário, disponibilidade, id_profissional) VALUES (?, ?, ?)", [horário, disponibilidade, {id_profissional: user_id}])
  ...
  async update(request, response) { //funcionalidade de atualização do usuário
    ...
    const horário = await database.get("SELECT * FROM horáriosSegunda WHERE id = (?)", [id]) //selecionando todos as colunas da linha que tem o respectivo id
