...
class HoráriosTerçaController {
  ...
  async create(request, response) {
    ...
    await database.run("INSERT INTO horáriosQuarta (horário, disponibilidade, id_profissional) VALUES (?, ?, ?)", [horário, disponibilidade, {id_profissional: user_id}])
  ...
  async update(request, response) { //funcionalidade de atualização do usuário
    ...
    const horário = await database.get("SELECT * FROM horáriosQuarta WHERE id = (?)", [id]) //selecionando todos as colunas da linha que tem o respectivo id
  ...
  async index(request, response) {
    const user_id = request.user.id;
    const horários = await knex("horáriosQuarta").where({ user_id }).first()
